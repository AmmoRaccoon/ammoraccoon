...
# Per-caliber category URLs now live in caliber_paths/velocity.json (expansion
# #4 Step-2 migration) — transcribed verbatim, parity-proven byte-identical.
# (WooCommerce store; the /collections/ paths map to WC product_cat archives,
# not Shopify collections.) entry['url'] is a drop-in for the old path.
CALIBER_PATHS = load_caliber_paths('velocity')
...
def scrape_category_page(page, entry, caliber_norm, caliber_display, retailer_id, seen_ids):
    ...
    if resp and resp.status >= 400:
        ...
        return 0, 0, True
    # Redirect guard (NEW 2026-06-14, expansion #4 Step-2 — Velocity had none
...
def scrape():
    ...
    report_empty_first_pages(empty_handles, 'Velocity Ammo Sales')
