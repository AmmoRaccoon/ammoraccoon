-- 035_homepage_segment_aggregates_3006_floor.sql
-- Adds the .30-06 Springfield per-caliber price floor (0.20) to homepage_segment_aggregates().
--
-- WHY: caliber #13 (.30-06 Springfield) shipped ACTIVE (calibers.json +
-- market_bounds_order append-last) and regenerated migrations/gen/caliber_floors.values.sql
-- with a new ('30-06', 0.20) row. Per the gen-file contract that block is never applied
-- directly -- it reconciles to the live DB through THIS numbered migration. .30-06's
-- 0.20 floor mirrors lib/priceBounds.js PER_CALIBER_FLOOR (regenerated alongside).
--
-- HOW: 034's body verbatim with ONE row appended to the caliber_floors VALUES
-- table (('30-06', 0.20)). Function signature UNCHANGED (8 params); every existing
-- floor row byte-identical; ph_floor logic untouched.
--
-- Unlike 10mm (034) which pre-staged a floor for a then-staged caliber, .30-06 is
-- ACTIVE at apply time, so this floor takes effect on real .30-06 listings as they
-- land. refresh_homepage_segment_aggregates_cache() (031/032) needs NO change
-- (signature unchanged; p_price_floor=0.10 fallback passes through).
--
-- Idempotent: CREATE OR REPLACE; re-running is a no-op replace.
-- Revert: re-run migration 034's body verbatim.
