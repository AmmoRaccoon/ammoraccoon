-- 036_homepage_segment_aggregates_270win_floor.sql
-- Adds the .270 Winchester per-caliber price floor (0.20) to homepage_segment_aggregates().
--
-- WHY: caliber #14 (.270 Winchester) shipped ACTIVE (calibers.json +
-- market_bounds_order append-last) and regenerated migrations/gen/caliber_floors.values.sql
-- with a new ('270win', 0.20) row. Per the gen-file contract that block is never applied
-- directly -- it reconciles to the live DB through THIS numbered migration. .270 Win's
-- 0.20 floor mirrors lib/priceBounds.js PER_CALIBER_FLOOR (regenerated alongside).
--
-- HOW: 035's body verbatim with ONE row appended to the caliber_floors VALUES
-- table (('270win', 0.20)). Function signature UNCHANGED (8 params); every existing
-- floor row byte-identical; ph_floor logic untouched.
--
-- .270 Win is ACTIVE at apply time, so this floor takes effect on real .270 Win
-- listings as they land. refresh_homepage_segment_aggregates_cache() (031/032) needs
-- NO change (signature unchanged; p_price_floor=0.10 fallback passes through).
--
-- Idempotent: CREATE OR REPLACE; re-running is a no-op replace.
-- Revert: re-run migration 035's body verbatim.
