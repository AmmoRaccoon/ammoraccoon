...
from scraper_lib import CALIBERS, now_iso, clean_title, normalize_caliber, with_stock_fields, parse_purchase_limit, sanity_check_ppr, parse_bullet_type, parse_brand, mark_retailer_scraped, insert_price_history, load_caliber_paths, category_redirected, report_empty_first_pages

SUPABASE_URL = os.environ["SUPABASE_URL"]
SUPABASE_KEY = os.environ["SUPABASE_KEY"]
RETAILER_ID = 13
SITE_BASE = "https://ammo.com"
USER_AGENT = (
    "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
    "(KHTML, like Gecko) Chrome/122.0 Safari/537.36"
)

# Per-caliber category paths now live in caliber_paths/ammocom.json
...
# 223-556 splits into two upstream pages (.223 Rem + 5.56x45 are
# separate Ammo.com categories); both bucket to 223-556 and seen_ids
# dedups overlap.
CALIBER_PATHS = load_caliber_paths('ammocom')
        ...
        while True:
            ...
            for card in cards:
                try:
                    ...
                    cal_disp, cal_norm = normalize_caliber(clean_title(title))
                    if not cal_norm:
                        continue
                    ...
                    if not sanity_check_ppr(ppr, price, rounds, context=title[:60], caliber=cal_norm):
                        continue
...
async def scrape():
    async with async_playwright() as p:
        ...
        report_empty_first_pages(empty_handles, 'Ammo.com')
